"""
PostgreSQL layer for the Policy Node — SQLAlchemy 2.0 models, session factory,
and realistic enterprise seed data.

DATABASE_URL controls the backend (PostgreSQL in production). Example:
  postgresql+psycopg://sentinelx:sentinelx@localhost:5432/sentinelx
A DATABASE_URL override is honored so the module is testable via dependency injection.
"""
...
# --------------------------------------------------------------------------- #
# Engine / session factory (dependency-injectable)
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Realistic enterprise seed data (aligned with the Enterprise Security Policy)
# --------------------------------------------------------------------------- #
_COMPLIANCE = [
...
_APPROVAL_RULES = [
    ApprovalRule(department="Finance", criticality="critical", action="disable_access_key",
                 approver="Security Manager", approval_level=2),
    ApprovalRule(department="Finance", criticality="critical", action="rotate_credentials",
                 approver="Security Manager", approval_level=2),
    ApprovalRule(department="Data", criticality="critical", action="revoke_access",
                 approver="Data Protection Officer", approval_level=3),
    ApprovalRule(department="Cloud", criticality="medium", action="restrict_access",
                 approver="Cloud Team Lead", approval_level=1),
    ApprovalRule(department="IT", criticality="critical", action="terminate_instance",
                 approver="CISO", approval_level=3),
    ApprovalRule(department="IT", criticality="high", action="isolate_instance",
                 approver="Security Manager", approval_level=2),
    ApprovalRule(department="IT", criticality="critical", action="enforce_root_lockdown",
                 approver="CISO", approval_level=3),
    ApprovalRule(department="Cloud", criticality="high", action="rotate_secrets",
                 approver="Cloud Security Lead", approval_level=2),
]
...
def seed_if_empty(session_factory: Callable[[], Session]) -> None:
    """Idempotently populate all tables with realistic enterprise records."""
    with session_factory() as s:
        if s.scalar(select(Compliance).limit(1)) is not None:
            return
        s.add_all(_COMPLIANCE)
        s.flush()
        s.add_all(_POLICIES + _APPROVAL_RULES + _ESCALATION)
        s.commit()
