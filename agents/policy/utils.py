"""Shared utilities: LLM helper (single instance), extraction, JSON parsing, timing."""
...
_ASSET_ALIASES = {"iam":"iam_user","iam_user":"iam_user","user":"iam_user","identity":"iam_user",
    "s3":"s3","bucket":"s3","ec2":"ec2","instance":"ec2","lambda":"lambda","function":"lambda",
    "root":"root_account","root_account":"root_account","rds":"rds","eks":"eks"}

def normalize_asset_type(raw: Optional[str]) -> Optional[str]:
    if not raw: return None
    key = str(raw).strip().lower().replace(" ", "_")
    for token, canon in _ASSET_ALIASES.items():
        if token in key: return canon
    return key
...
def derive_action(asset_type: Optional[str], incident: dict) -> Optional[str]:
    ...
    return {"iam_user":"disable_access_key","s3":"revoke_access","ec2":"isolate_instance",
            "lambda":"rotate_secrets","root_account":"enforce_root_lockdown"}.get(asset_type or "", None)

def severity_from_criticality(criticality: Optional[str], incident: dict) -> str:
    if incident.get("severity"): return str(incident["severity"])
    return {"critical":"Sev1","high":"Sev2","medium":"Sev3","low":"Sev4"}.get(str(criticality or "").lower(), "Sev3")
...
class Timer:
    ...
    def __exit__(self, *a): self.ms = round((time.perf_counter() - self._t) * 1000, 1)
